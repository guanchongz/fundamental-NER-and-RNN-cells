...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        n_features=int(window_size*2+1)
        for i in range(length_sentence):
            tup=(sentence[i:i+n_features],labels[i])
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings=tf.reshape(-1,self.config.n_window_features*self.config.embed_size)
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        h=tf.nn.relu(tf.nn.xw_plus_b(x,W,b1))
        h_drop=tf.nn.dropout(h,1-dropout_rate)
        pred=tf.nn.xw_plus_b(h_drop,U,b2)
        ### END YOUR CODE
        return pred
    ...
    def __init__(self, helper, conig, pretrained_embeddings, report=None):
