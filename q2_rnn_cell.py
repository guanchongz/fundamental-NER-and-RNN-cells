    ...
    with tf.Graph().as_default():
        with tf.variable_scope("test_rnn_cell"):
            ...
            with tf.variable_scope("rnn"):
                tf.get_variable("W_x", initializer=np.array(np.eye(3,2), dtype=tf.float32))
                tf.get_variable("W_h", initializer=np.array(np.eye(2,2), dtype=tf.float32))
                tf.get_variable("b",  initializer=np.array(np.ones(2), dtype=tf.float32))
